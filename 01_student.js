...
function structureOk(code, lesson){
  ...
  if(!/\bprint\s*\(/.test(lc)) need.push("in kết quả bằng print()");
  ...
  if(hay.includes("hàm") || hay.includes("def")){
  ...
  // fixed-output tasks (no input): must print exact phrase somewhere
  if(inp === "(không có)" && lesson && lesson.tests && lesson.tests[0] && typeof lesson.tests[0].expected === "string"){
    const exp = normalizeLoose(lesson.tests[0].expected);
    if(exp && exp.length <= 40){
      if(!lc.includes(exp.toLowerCase())) need.push(`in đúng nội dung "${exp}"`);
    }
  }
...
async function runTests(){
  ...
  for(let i=0;i<current.tests.length;i++){ 
    ...
    const ok = normalizeLoose(out) === normalizeLoose(tc.expected);
    if(ok){ pass++; details.push(`✅ Test ${i+1}: PASS`); }
    else{ details.push(`❌ Test ${i+1}: FAIL\n- Output: ${JSON.stringify(out)}\n- Expected: ${JSON.stringify(tc.expected)}`); }
  }

  // PASS nới lỏng: chỉ cần đúng câu lệnh + cấu trúc (và chạy không lỗi)
  let structurePass = false;
  if(pass < current.tests.length && !hadError){
    try{
      const st = structureOk(code, current);
      if(st && st.ok){
        structurePass = true;
        pass = current.tests.length;
        details.push(`\n✅ PASS theo cấu trúc (không bắt buộc khớp 100% output).`);
      } else if(st && st.need && st.need.length){
        details.push(`\nℹ️ Thiếu cấu trúc để PASS: ${st.need.slice(0,4).join(', ')}`);
      }
    }catch(e){}
  }
...
function initEditor(){
  ...
  try{
    ...
    try{ window.__editor = editor; window.__customPythonHint = customPythonHint; }catch(e){}
